...
// bool == false indicates a successful adding
...
func handlerLogin(w http.ResponseWriter, r *http.Request) {
	...
	// Get user info from client request
	...
	// generate token for user, this is the payload field
...
func handlerSignup(w http.ResponseWriter, r *http.Request) {
	...
	// Get User infomation from client
	decoder := json.NewDecoder(r.Body)
	var user User
	if err := decoder.Decode(&user); err != nil {
		http.Error(w, "Cannot decode user data from client", http.StatusBadRequest)
	...
	// Save new user to Elasticsearch
	exists, err := addUser(&user)
	if err != nil {
	...
	fmt.Printf("User added successfully: %s", user.Username)
	return
}
